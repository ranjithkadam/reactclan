...
export default function Register() {
  ...
  const validateFields = () => {
    ...
    // Name Validation
    if (!/^[A-Z][a-z]{2,10}$/.test(formData.name)) {
      errors.name = "First name must be 3-10 characters long and start with a capital letter";
    }
     
    //  phone Validation
    if (!/^[6-9][0-9]{9}$/.test(formData.phone)) {
      errors. phone = " phone number must be 10 digits long and start with 6-9";
    }
    // Email Validation
  ...
  const handleSubmit = (e) => {
    ...
    let stringfyData= JSON.stringify(formData);
